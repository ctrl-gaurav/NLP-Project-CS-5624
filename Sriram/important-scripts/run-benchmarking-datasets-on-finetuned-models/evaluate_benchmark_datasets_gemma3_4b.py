...
from transformers import LogitsProcessorList, TemperatureLogitsWarper, TopPLogitsWarper, TopKLogitsWarper, RepetitionPenaltyLogitsProcessor, InfNanRemoveLogitsProcessor

# ================= CONFIGURATION =================
MODEL_PATH = "/home/sriramsrinivasan/SFT/finetuned-gemma-3-4b-it/final_model"  # Path to your fine-tuned model
DEVICE = "cuda" if torch.cuda.is_available() else "cpu"
MAX_NEW_TOKENS = 512  # Adjusted from 1024 - experiment with this
TEMPERATURE = 0.1
DATASETS_TO_EVALUATE = ["gsm_8k", "arc_challenge", "commonsense_qa"]
# DATASETS_TO_EVALUATE = ["arc_challenge", "commonsense_qa"]
...
def load_model(model_path, device="cuda"):
    ...
    try:
        ...
        # If PEFT fails, load as a regular model
...
def format_prompt_gsm8k(question, tokenizer):
    """Format the prompt for GSM8K using the chat template"""
    messages = [
        {"role": "user", "content": f"Can you solve this math problem?\nYour final answer must be in the format \\boxed{{answer}} at the end.\n\nProblem: {question}"}
    ]
    return tokenizer.apply_chat_template(messages, tokenize=False, add_generation_prompt=True)

def format_prompt_arc(question, choices, tokenizer):
    """Format the prompt for ARC challenge using the chat template"""
    choice_str = "\n".join([f"{chr(ord('A') + i)}. {choice}" for i, choice in enumerate(choices)])
    messages = [
        {"role": "user", "content": f"Answer the following multiple choice question:\n{question}\n{choices}\nYour answer should be one of the choices provided."}
    ]
    return tokenizer.apply_chat_template(messages, tokenize=False, add_generation_prompt=True)

def format_prompt_csqa(question, choices, tokenizer):
    """Format the prompt for CommonSenseQA using the chat template"""
    choice_str = "\n".join([f"{chr(ord('A') + i)}. {choice}" for i, choice in enumerate(choices)])
    messages = [
        {"role": "user", "content": f"Answer the following question by choosing the best option:\n{question}\n{choices}\nYour answer should be a single letter corresponding to the correct choice."}
    ]
    return tokenizer.apply_chat_template(messages, tokenize=False, add_generation_prompt=True)

def generate_answer(model, tokenizer, prompt, max_new_tokens=1024, temperature=0.7):  # Adjusted max_new_tokens and temperature
        ...
        outputs = model.generate(
            **inputs,
            ...
            logits_processor=LogitsProcessorList([
                InfNanRemoveLogitsProcessor(),  # Keep this for safety
            ])
        )
...
def evaluate_model(model, tokenizer, dataset_name, split="test", sample_limit=None):
    ...
    if dataset_name == "gsm_8k":
        ...
        #for i in range(min(5, len(dataset))):  # Print the first 5 prompts
        #    question = dataset[i]["question"]
        #    prompt = format_prompt_fn(question, tokenizer)
        #    print(f"GSM8K Prompt {i+1}:")
        #    print(prompt)
        ...
        #    print(f"ARC Challenge Prompt {i+1}:")
        #    print(prompt)
