...
def parse_arguments():
    ...
    parser.add_argument('--model_path', required=True, help='Hugging Face model ID')
...
def create_output_directory(args):
    ...
    if args.output_dir:
        ...
        model_name = args.model_path.split('/')[-2] # Extract model name from path
...
def main():
    ...
    # Check if model_path is provided (won't be needed if --version was called)
    ...
    model_handler = ModelHandler(
        ...
        device="cuda" if args.tensor_parallel_size >= 1 else "cpu",
