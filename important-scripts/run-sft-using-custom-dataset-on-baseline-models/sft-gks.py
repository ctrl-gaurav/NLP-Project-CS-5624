...
"""
Qwen 3B Fine-tuning Script for Debate Consensus Dataset
Optimized for fine-tuning on consensus responses from multi-agent debates
"""
...
class FinetuningConfig:
    """Configuration for fine-tuning Qwen model"""
    ...
    # Data Parameters
    data_path: str = field(
        default="final_training_gsm_plus_qwen3b.json",
        metadata={"help": "Path to the JSON file containing the fine-tuning data"}
    )
    output_dir: str = field(
        default="./finetuned-qwen-debate",
    ...
    def __post_init__(self):
        ...
        if self.wandb_run_name is None:
            ...
            self.wandb_run_name = f"qwen3b_finetune_ft_{timestamp}"
...
def load_and_prepare_data(config: FinetuningConfig):
    ...
    for item in data:
        ...
        # Using the 'final_consensus_response' as target for fine-tuning
        response = item.get('final_consensus_response', '')
        
        if not response:  # Fallback to consensus_response if final doesn't exist
            response = item.get('consensus_response', '')
...
def format_prompt(question, tokenizer):
    """Format the prompt for Qwen 2.5 model (adjust as needed)"""
    # For Qwen2.5 Instruct format
    messages = [
...
def tokenize_data(dataset, tokenizer, config: FinetuningConfig):
    ...
    def tokenize_function(examples):
        ...
        prompts = [format_prompt(q, tokenizer) for q in examples["question"]]
        
        # Combine prompts with responses (for a causal LM setting)
        completions = []
        for prompt, response in zip(prompts, examples["response"]):
            completions.append(f"{prompt}{response}{tokenizer.eos_token}")
        ...
        # Mask out prompt tokens for loss calculation (optional)
...
def create_peft_config(config: FinetuningConfig):
    ...
    if config.use_lora:
        # Define target modules for Qwen
        # These are typical attention modules, adjust as needed for Qwen architecture
